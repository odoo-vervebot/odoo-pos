...
class PosDashboard(models.Model):
    ...
    def get_department(self, option):
        ...
        if option == 'pos_hourly_sales':
            ...
            #            company_id) + ''' group by date_month order by date_month desc '''
            query = '''select  EXTRACT(hour FROM date_order) 
                       as date_month,sum(amount_total) from pos_order where  
            ...
            query = '''select  date_order::date as date_month,EXTRACT(Day FROM date_order::date) int_date,sum(amount_total) from pos_order where 
             EXTRACT(month FROM date_order::date) = EXTRACT(month FROM CURRENT_DATE) AND pos_order.company_id = ''' + str(
                company_id) + '''  group by date_month, int_date order by int_date desc '''
            label = 'DAYS'
        else:
            query = '''select TO_CHAR(date_order,'MON')date_month, EXTRACT(month FROM date_order::date) int_month, sum(amount_total) from pos_order where
             EXTRACT(year FROM date_order::date) = EXTRACT(year FROM CURRENT_DATE) AND pos_order.company_id = ''' + str(
                company_id) + ''' group by date_month, int_month order by int_month  desc'''
            label = 'MONTHS'
